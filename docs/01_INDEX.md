...
基于 NapCat OneBot11 协议的 Python QQ 机器人 SDK。

## ⚡ 快速开始

```bash
pip install -r requirements.txt
cp config.yaml.example config.yaml   # 编辑 config.yaml 配置连接信息
python run.py                         # 启动机器人
```
...
**新手入门**：02_OVERVIEW → 03_QUICK_START → 04_CONFIG_GUIDE  
**插件开发**：06_PLUGIN_DEVELOPMENT → 05_API  
**深入理解**：阅读 `sdk/` 源码和 `plugins/` 示例
